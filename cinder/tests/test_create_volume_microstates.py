...
class fake_db(object):
    ...
    def micro_states_create(self, *args, **kwargs):
        self.micro_states_dict = {
            'id': 'fake_id',
            'resource_id': 'fake_resource_id',
            'state': 'fake_state'
        }
        return self.micro_states_dict

    def micro_states_get(self, *args, **kwargs):
        return self.micro_states_dict

    def micro_states_update(self, *args, **kwargs):
        new_state = {'state': 'fake_new_state'}
        self.micro_states_dict.update(new_state)
        return self.micro_states_dict
...
class CreateVolumeMicrostatesTestCase(test.TestCase):
    ...
    def test_entry_create_task(self):
        ...
        create_api = volume.flows.api.create_volume.EntryCreateTask(
            fake_db())
        fake_volume = {'volume_id': 'fake_resource_id',
    ...
    def test_volume_cast_task(self):
        ...
        cast_task_api = volume.flows.api.create_volume.VolumeCastTask(
            ...
            fake_db())
        micro_state = fake_db().micro_states_get()
        cached_micro_state = {}
        cached_micro_state['id'] = micro_state['id']
        cached_micro_state['resource_id'] = micro_state['resource_id']
        cached_micro_state['state'] = micro_state['state']
        cast_task_api._post_execute(self.ctxt, micro_state)
        post_micro_state = fake_db().micro_states_get()
        self.assertEqual(cached_micro_state['id'], post_micro_state['id'])
        self.assertEqual(cached_micro_state['resource_id'],
                         post_micro_state['resource_id'])
